<?php
   ...
   ?>
<div class="extra_content <?php echo esc_attr($settings['extra_content_types']); ?> <?php echo esc_attr($settings['dark_white']); ?>" <?php if($settings['transitions_enable'] == 'yes'):  ?>  data-aos="fade-up" data-aos-delay="<?php echo esc_html($settings['transitions']); ?>"  data-aos-offset="0" <?php endif;?>>
   <?php if($settings['extra_content_types'] == 'authour_box'):?>
   <div class="authour_box_content">
      <?php if(!empty($settings['sign_image'])): ?>
      <div class="image">
         <img src="<?php echo esc_url($settings['sign_image']['url']); ?>" class="img-fluid sign_image" alt="<?php esc_attr_e('authour Image', 'creote-addons'); ?>" />
      </div>
      <?php endif; ?>
      <?php if(!empty($settings['authour_text'])): ?>
      <div class="text">
         <h6><?php echo wp_kses($settings['authour_text'] , $allowed_tags) ?></h6>
      </div>
      <?php endif; ?>
   </div>
   <?php elseif($settings['extra_content_types'] == 'download_docs'):?>
   <div class="download_box_content">
      <?php if(!empty($settings['download_text'])): ?>
      <a href="<?php echo esc_url($settings['download_link']);?>" download>
      <?php echo wp_kses($settings['download_text'] , $allowed_tags) ?>
      <i class="icon-download-symbol"></i>
      </a>
      <?php endif; ?>
   </div>
   <?php elseif($settings['extra_content_types'] == 'authour_box_two'):?>
   <div class="authour_box_content two">
      <?php if(!empty($settings['authour_image'])): ?>
      <div class="image">
         <img src="<?php echo esc_url($settings['authour_image']['url']); ?>" class="img-fluid authour_image" alt="<?php esc_attr_e('authour Image', 'creote-addons'); ?>" />
      </div>
      <?php endif; ?>
      <div class="text">
         <?php if(!empty($settings['authour_text_two'])): ?>
         <h6><?php echo wp_kses($settings['authour_text_two'] , $allowed_tags) ?></h6>
         <?php endif; ?>
         <?php if(!empty($settings['description'])): ?>
         <p><?php echo wp_kses($settings['description'] , $allowed_tags) ?></p>
         <?php endif; ?>
         <?php if(!empty($settings['sign_image_two'])): ?>
         <img src="<?php echo esc_url($settings['sign_image_two']['url']); ?>" class="img-fluid sign_image_two" alt="<?php esc_attr_e('authour Image', 'creote-addons'); ?>" />
         <?php endif; ?>
      </div>
   </div>
   <?php elseif($settings['extra_content_types'] == 'image_with_content'):?>
   <div class="simple_image">
      <img src="<?php echo esc_url($settings['img_tit']['url']); ?>" alt="img" />
      <h2> <?php echo wp_kses($settings['title_img'] , $allowed_tags) ?> </h2>
   </div>
   <?php endif; ?>
</div>
<?php

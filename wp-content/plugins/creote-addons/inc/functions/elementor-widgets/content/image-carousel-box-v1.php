...
?>
<section class="image_box_carousel_content <?php echo esc_attr($settings['arrow_enable_disable']); ?> <?php if($settings['object_fit_cover'] == 'yes'): ?> image_covered <?php endif; ?>">
   <div class="theme_carousel owl-theme owl-carousel" data-options='{"loop": false, "margin": 20, "autoheight":true, "lazyload":true, "nav": true, "dots": false, "autoplay": false, "autoplayTimeout": 6000, "smartSpeed": 300, "responsive":{ "0" :{ "items": "1" }, "600" :{ "items" : "2" }, "768" :{ "items" : "2" } , "992":{ "items" : "3" }, "1200":{ "items" : "<?php echo esc_attr($settings['item_to_display']); ?>" }}}'>
      <?php foreach($settings['image_carousel_repeater'] as $key => $image_carousel_repeater): 
         $target = $image_carousel_repeater['link']['is_external'] ? ' target="_blank"' : '';
         $nofollow = $image_carousel_repeater['link']['nofollow'] ? ' rel="nofollow"' : ''; 
         
         ?>
      <?php // style one ?>
      <?php if($settings['image_box_styles'] == 'style_one'):?>
      <div class="mg_image_box">
         <?php if(!empty($image_carousel_repeater['image']['url'])): ?>
         <div class="image_box">
            <img src="<?php echo esc_url($image_carousel_repeater['image']['url']); ?>" class="img" alt="image" />
            <a href="<?php echo esc_url($image_carousel_repeater['link']['url']);?>" class="ab_link" <?php echo esc_attr($target); ?> <?php echo esc_attr($nofollow); ?>>
         ...
         <h2>  <a href="<?php echo esc_url($image_carousel_repeater['link']['url']);?>"  <?php echo esc_attr($target); ?> <?php echo esc_attr($nofollow); ?>>
